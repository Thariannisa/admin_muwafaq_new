...
from .model import MateriFathurahman, materiFathurahman_KIND
...
def getId(id):
    ...
    if id != None:
        ...
        satu_hasil = client.get(client.key(materiFathurahman_KIND, id))
        if satu_hasil != None:
            return MateriFathurahman(id=satu_hasil.id,
...
def cari(id):
    ...
    if id is not None:
        ...
        key_materi = client.key(materiFathurahman_KIND, id)
        ...
        materi_fathurahman = MateriFathurahman(id=hasil.id,
                                               idTema=hasil["idTema"],
                                               judul=hasil["judul"],
        ...
        data_materi.append(materi_fathurahman)
...
def ubah_tulisan(id, edit_tulisan):
    ...
    key_materifathurahman = client.key(materiFathurahman_KIND, id)
    #  ambil hasil carinya
    hasil = client.get(key_materifathurahman)
    # jika tidak ditemukan, bangkitkan exception
    if hasil is None:
        raise EntityNotFoundException(
            f"Tidak ada materi dengan id: {id}.")
    ...
    client.put(hasil)
    # kembalikan data penindaklanjut
    return MateriFathurahman(id=id,
                             idTema=hasil["idTema"],
