...
from .model import MateriTajwid, materiTajwid_KIND
...
def getId(id):
    ...
    if id != None:
        ...
        satu_hasil = client.get(client.key(materiTajwid_KIND, id))
        if satu_hasil != None:
            return MateriTajwid(id=satu_hasil.id,
...
def cari(id):
    ...
    if id is not None:
        ...
        key_materi = client.key(materiTajwid_KIND, id)
        ...
        materi_tajwid = MateriTajwid(id=hasil.id,
                                     idTema=hasil["idTema"],
                                     judul=hasil["judul"],
        ...
        data_materi.append(materi_tajwid)
...
def ubah_tulisan(id, edit_tulisan):
    ...
    key_materitajwid = client.key(materiTajwid_KIND, id)
    #  ambil hasil carinya
    hasil = client.get(key_materitajwid)
    # jika tidak ditemukan, bangkitkan exception
    if hasil is None:
        raise EntityNotFoundException(
            f"Tidak ada materi dengan id: {id}.")
