...
from .model import MateriUmum, materiUmum_KIND
...
def getId(id):
    ...
    if id != None:
        ...
        satu_hasil = client.get(client.key(materiUmum_KIND, id))
        if satu_hasil != None:
            return MateriUmum(id=satu_hasil.id,
...
def cari(id):
    ...
    if id is not None:
        ...
        key_materi = client.key(materiUmum_KIND, id)
        ...
        materi_umum = MateriUmum(id=hasil.id,
                                 idTema=hasil["idTema"],
                                 judul=hasil["judul"],
        ...
        data_materi.append(materi_umum)
...
#     key_materiumum = client.key(materiUmum_KIND, id)
#     #  ambil hasil carinya
#     hasil = client.get(key_materiumum)
#     # jika tidak ditemukan, bangkitkan exception
#     if hasil is None:
#         raise EntityNotFoundException(
#             f"Tidak ada materi dengan id: {id}.")
