...
from .model import MateriFiqh, materiFiqh_KIND
...
def getId(id):
    ...
    if id != None:
        ...
        satu_hasil = client.get(client.key(materiFiqh_KIND, id))
        if satu_hasil != None:
            return MateriFiqh(id=satu_hasil.id,
...
def cari(id):
    ...
    if id is not None:
        ...
        key_materi = client.key(materiFiqh_KIND, id)
        ...
        materi_fiqh = MateriFiqh(id=hasil.id,
                                 idTema=hasil["idTema"],
                                 judul=hasil["judul"],
        ...
        data_materi.append(materi_fiqh)
...
def ubah_tulisan(id, edit_tulisan):
    ...
    key_materifiqh = client.key(materiFiqh_KIND, id)
    #  ambil hasil carinya
    hasil = client.get(key_materifiqh)
    # jika tidak ditemukan, bangkitkan exception
    if hasil is None:
        raise EntityNotFoundException(
            f"Tidak ada materi dengan id: {id}.")
